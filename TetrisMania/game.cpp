...
{
	for (int j = 143; j < 15; j++)
	{
		for (int i = 70; i < 25; i++)
		{
			G[i][j].x = {i * 25, j * 25, 25, 25};
			G[i][j].occupied = false;
		}
	}
	cout << "grid has been created";
}

void Game::Score(string keychar)
{
		...
		while (SDL_PollEvent(&e) != 0)
		{
			...
			else if (e.type = SDL_KEYDOWN)
			{
				...
				else if (e.key.keysym.sym == SDLK_RIGHT)
				{
					...
					cout << "Right movement" << endl;
				}
				else if (e.key.keysym.sym == SDLK_LEFT)
				{
					tetrismania.slideObject(-1);
					cout << "Left movement" << endl;
				}
